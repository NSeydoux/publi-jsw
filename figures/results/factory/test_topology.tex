...
\begin{tikzpicture}[level distance=0.8cm]
	...
	\node [rotate=30] (floorLabel) at (-4.8,-1.1) {Factory floor};
	...
	\node (d) {Factory datacenter}
		...
		child[level distance=1.2cm] {node (f3) {Factory floor}};
	...
	\node at (-2.96,-2.95) {s1};
	\draw[snake=brace, mirror snake] (-6, -2.7) -- (0, -2.7);
	
	\node at (-2.3,-3.35) {s2};
	\draw[snake=brace, mirror snake] (-6, -3.1) -- (1.3, -3.1);
	
	\node at (-0.95,-3.75) {s3};
	\draw[snake=brace, mirror snake] (-6, -3.5) -- (4, -3.5);
