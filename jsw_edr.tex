...
\removed{An example of enriched observation is available online%\footnote{\url{https://w3id.org/laas-iot/rules/observations/enriched_data.ttl}}.
...
Such node has a set $\mathcal{P}$ of children (either sensors or other Fog nodes) \textbf{partially producing the rule head}. 
...
The data, of property type $\rho_{i}$, is in the first place sent to $n_p\in \added{Parent(n)}$ if it is a consumer of $\rho_{i}$. 
