...
%\usepackage[hyphens]{url} % must be before hyperref
%\usepackage{url}
...
The relation between a node $n_p$ and any $n_c\in Lower(n)$ is expressed with the triple \triplet{$n_p$}{\concept{lmu}{has\-Downstream\-Node}}{$n_c$}\footnote{Namespaces are available in Appendix \ref{app:namespaces}}$^,$\footnote{Individuals such as $n_p$ and $n_c$ are identified with an \gls{iri} in the triples}, based on a nomenclature presented in \cite{Seydoux2017}. 
...
In order to be announced towards child node $n_{child}$, the triple \triplet{$n_{parent}$}{$has\-Charac$}{$\nu$} is transformed into the following reified statement: 
...
\gls{edr} is designed for neighbor-to-neighbor rule and data propagation : a node only communicates with its direct neighbors in the hierarchy, either parent or children (with the exception of deduction delivery).
...
Let us assume that node $n$ has a child $n_{child}$, and that $n_{child}$ has a characteristic expressed by the triple \triplet{$n_{child}$}{$has\-Cha\-racteri\-stic$}{$\nu$}, that should be proxied towards $n_{parent}\in Upper(n)$.
...
Since $has\-Characteristic$ is such a property, the node $n$ updates its own representation towards its parent by sending the triple \triplet{$n$}{$has\-Characteri\-stic$}{$\nu$}, therefore proxying the capacity of $n_{child}$.
...
The SHACL standard is so that by default, when reasoning on a \gls{kb} containing SHACL shapes and rules, all of them are considered\footnote{See Section \textsection 4.3 of the recommendation \url{https://www.w3.org/TR/shacl/\#validation-definition}}.
...
Therefore, when a child node connects to its parent, it includes the triples denoting its productions in its self-description. 
...
If the parent node $n_{parent}$ was not a producer of the property type $\rho_t$, it includes a new triple in its \gls{kb} \triplet{$n_{parent}$}{\concept{edr}{produces\-Data\-On}}{$\rho_t$}, and forwards this triple to its own parent.
...
The interest of a node $n$ for a property type $\rho_t$ is represented in the \gls{kb} by the triple \triplet{$n_p$}{\concept{edr}{is\-Interested\-In}}{$\rho_t$}, and \triplet{\concept{edr}{is\-Interested\-In}}{\concept{rdf}{type}}{\concept{edr}{Some\-Children\-Announced\-Property}}.
...
\added{
As it is stated in Section \textsection \ref{subsubs:annouce}, the \edr approach, and by extension \edrt, is agnostic to the vocabulary used to describe nodes characteristics. 
For this use case, only a few properties were needed, as shown in Lst. \ref{lst:node_desc}. 
\lstinputlisting[float, caption=\added{Description of a node}, label=lst:node_desc]{listings/node_description.ttl}
The description of the node mainly encompasses its neighbors (with \texttt{lmu:} properties), its API (with \concept{iotl}{exposes}), and its capabilities (\concept{edr}{producesDataOn}).
...
Lst. \ref{lst:observation} shows a snippet of observed data, mainly described with the legacy version of the \texttt{ssn:} ontology. 
\lstinputlisting[float, caption=\added{Exchanged sensor observations}, label=lst:observation]{listings/node_description.ttl}
The proposed approach depends on the observation representation, that is why a standard vocabulary has been chosen. 
In future versions, \edr will be updated to be compliant with the updated version of SSN, SSN/SOSA\footnote{\url{http://www.w3.org/ns/sosa/}}. 
...
The last two headers are ad-hoc to ADREAM, and they are not used in this experiment, and the other are self-explanatory.
This raw data is enriched thanks to a SPARQL-Generate query, a sample of which is show in Lst. \ref{lst:enrichment}\footnote{\added{The complete query is available in the source code of EDR: node/\-queries/\-data/\-enrich\_data.sparql}}. 
\lstinputlisting[float, caption=\added{Enrichment query snippet}, label=lst:enrichment]{listings/enrichment.sparql}
}
