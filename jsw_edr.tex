...
The maturity of \gls{iot} communication technologies is fostering a wide variety of industrial and societal applications with responsiveness and privacy requirements, especially useful for home automation or industry 4.0 scenarios. 
However, the heterogeneity of \gls{iot} data and use cases raises interoperability issues constituting hurdles for the development of \gls{iot} service platforms and related applications. 
In order to tackle this issue, the \gls{sw} technologies and principles are as an interoperability enabler, by providing expressive vocabularies to describe data and manipulate information.
The domain at the interface between the \gls{sw} and the \gls{iot} is called the \gls{swot}, and it emergence is not trivial. 
Even though the \gls{swot} has been envisioned as soon as the fundamental article of the \gls{sw} \cite{Berners-Lee2001}, where smart agents interact with devices in the user's environment, concrete \gls{swot} relatizations where only proposed in recent years \cite{Pfisterer2011}.
%\Nicolas{La suite suppose un déploiement décentralisé, il faudrait l'introduire mieux}
...
The integration of the \gls{sw} stack to an \gls{iot} architecture is often centered on remote, powerful machines\Nicolas{ref}. 
\gls{iot} data is centralized on such machines before being processed using \gls{sw} technologies.
...
\gls{swot} architectures are characterized by devices pervasively distributed in the environment, with potentially limited communication capabilities. 
In order to transport data from these devices to Cloud nodes, multiple intermediary nodes are necessary.
However, this transfer introduces a delay in data processing, and decreases applications' responsivity.

One way of tackling the issues of the centralized, Cloud-based approach to the \gls{swot} is the distribution of the \gls{sw} stack on these middle nodes between the Cloud and the \gls{iot} devices.
Such approach is conform with the Fog computing paradigm \cite{Bonomi2012}, dedicated to storing and processing data \textbf{at the edge of the network}\cite{Patel2017}.
However, the Fog computing is not introduced as a paradigm meant to replace Cloud computing: its limited computing capabilities, as well as the locality of the scale of its deployments, are not suited to support Cloud computing use cases.
\textbf{Cloud and Fog computing are two complementary approaches}, supporting the deployment of complex \gls{swot} applications \cite{Sahni2017}.

The purpose of semantic processing is, thanks to knowledge captured in ontologies, to \textbf{process data in order to produce application-level information}. 
%In order to face the diversity of applications potentially interested in the data of an \gls{iot} deployment, 
Knowledge about the deployed \gls{iot} system and its environment can be a priori modelled by the system administrators.
However, it is also necessary to inject business-specific knowledge into the reasoning system, and this information is not known a priori.
It must therefore be modeled as bootstrapped\Nicolas{je ne suis pas trop sûr du terme, mais self-sufficient me semble inadapté} entities, inserted into the system at runtime.
Moreover, when considering a distributed approach, the integrality of the business knowledge might not be relevant in the context of all the nodes. 
If bundled into entities that can be exchanged node to node, application-level kowledge may be opportunistically distributed in the network. 
...
Rules are a common way to capture applicative logic: a rule is a self-contained representation of a logical process.
Due to these considerations, the contribution in this article considers rule-based reasoning: rules are used as representation of business logic, applied in a \gls{kb} capturing the environment of the node.

The contribution in this paper is \gls{edr}, a \textbf{generic approach to the dynamic distribution of rule-based reasoning in a Cloud-Fog \gls{iot} architecture}.
The work presented in this paper is an extension two conference articles, \cite{wi2018} and \cite{coopis2018}, where some aspects of \gls{edr} and its refinements have been introduced.
Novel work include the detailed presentation of the vocabulary enabling the genericity of \gls{edr} and the description of the usage of the Linked Open Rules \cite{Khandelwal2011} principles. 
...
The scientific challenge faced considers three characteristics of the distributed reasoning system: scalability, responsiveness, dynamicity. 
These characteristics are presented in details in \textsection \ref{sec:edr_characteristics}.
In \textsection \ref{sec:related_work_distribution}, existing work is introduced, to identify the added value of the present contribution.
The core contribution is detailed in \textsection \ref{sec:edr} and \ref{sec:edrpt}, and it is evaluated in \textsection \ref{sec:experimentations}.
This paper is concluded in \textsection \ref{sec:conclusion}.

\section{Desirable characteristics for the proposed solution}
\label{sec:edr_characteristics}

\Nicolas{Intro pourrie}
In order to capture the main characteristics of the contribution presented in \textsection \ref{sec:edr} and \textsection \ref{sec:edrpt}, an illustrative use case is introduced. 
This use case is also the drive for the evaluations in \textsection \ref{sec:experimentations}.
...
Moreover, the mobility of some sensors (\textit{e.g.,} workers wearable), combined to the modularity of the factory floors, are suitable for a dynamic solution adaptative to their evolution over time.
...
However, the difference of computing power between Cloud and Fog nodes should not be neglected: Cloud architectures' intrinsic capabilities enable a resource upscale which is not possible for Fog architectures.
...
In the proposed use case, the rules deployed in the system are used to detect potentially harmul situations, requiring the inferred notifications to be received by the control center as soon as possible. 
...
Therefore, \textbf{responsivity} is another characteristic we want to observe in our contribution.
...
Instead of funnelling all the data towards the Cloud before inferring higher level information, combining Fog computing and direct communication between Fog nodes and applications should enable a faster notification delivery.
...
Moreover, some of the considered devices are attached to workers that are mobile: they will be connected to different machines over time, leading to a dynamic network topology.
...
In our litterature search, we identified two main types of rules associated to the \gls{swot} \cite{Boley2007}:
\begin{itemize}
	\item \textbf{Production rules}, or deduction rules, in which preconditions are expressed as a logical expression, and postconditions are new knowledge which are the logical consequence of preconditions. Schematically, production rules can be represented by IF-THEN structures.
	...
	Such actions are not limited to knowledge inference: they can be instantiated by running a piece of code. Schematically, \gls{eca} rules can be represented by WHEN-IF-DO structures. 
\end{itemize}
...
\Nicolas{cf commentaire, voir si garder pour l'ouverture}%The authors also introduce an ad-hoc approach to lowering (\textit{c.f.} Section \textsection \ref{par:lowering}), with dedicated adapters transforming rich actuations representations into raw commands to be sent to actuators.
...
The contribution introduced in \cite{IoannisChatzigiannakis129} uses the same \gls{eca} model as IFTTT , but extends the rules expressiveness by using \gls{sw} formalisms, namely SWRL and SPARQL.
...
\gls{edr} differs from previous proposals by different aspects captured in the requirements described in Section \textsection \ref{sec:edr_characteristics}:
\begin{itemize}
	\item The \textbf{genericity} of the approach, enabling its adaptation to various application-level strategies.
	\item The \textbf{locality} of the knowledge involved in the rule deployment: each node only considers its own knowledge representation when propagating a rule.
	\item The \textbf{dynamism} of rule deployment in the \gls{swot} system at runtime, constantly adapting to the state of the topology in an event-driven behavior.
\end{itemize} 

\section[EDR, approach for distributed reasoning]{EDR, a generic approach to dynamically distributed rule-based reasoning}
\label{sec:edr}
...
Once the main components of the \gls{edr} mechanisms are described, the relationship between \gls{edr}'s characteristics and semantic Fog computing are discussed in Section \textsection \ref{sec:edr_characteristics}.
\Nicolas{cf commentaire : voir si positionnement par rapport à edr nécessaire ici}

\subsection{Assumptions on the underlying architecture}
\label{subs:edr_asumptions}

\gls{edr} is based on the hypothesis of a \textbf{hierarchical network topology}: nodes are organized in a tree-like structure, and only communicate with neighboring nodes.
...
%Such hierarchical topology is related to the Cloud-Fog-Device architectural pattern: the tree root is a Cloud node, leaves are devices, and nodes in between are Fog nodes.
...
It is assumed therefore that Fog nodes can communicate with applications directly.
...
In order to ensure decentralization, the algorithm of the \gls{edr} approach is executed in parallel on each node able to perform reasoning in the topology, \textit{i.e.} Cloud node and semantic-computing-enabled Fog nodes. 
\gls{edr} considers a step-by-step rule and data propagation: each node only communicates with its direct neighbors within the \gls{iot} network, \textit{i.e.} its parent and children in the hierarchical topology.
This design choice allows to reduce the nodes' knowledge of the topology to a limited subset of the complete deployment. 
...
\textbf{The \gls{edr} approach itself is agnostic to the deployment strategy}, which is defined by the rule implementor: that is why we qualify \gls{edr} as \textbf{generic}. 
...
\subsection{A deployment mechanism based on a dedicated vocabulary}
...
The relation between a node $n_p$ and its child $n_c$ is expressed with the triplet \triplet{$n_p$}{lmu:hasDownstreamNode}{$n_c$}\footnote{Namespaces are available on \url{http://prefix.cc}\Nicolas{rajouter lmu sur prefix}}\footnote{Individuals such as $n_p$ and $n_c$ are identified with an \gls{iri} in the triplets}, based on a nomenclature presented in \cite{Seydoux2017}. 
The inverse relation exists, to express the connection between a node $n_c$ and its parent $n_p$: \triplet{$n_c$}{lmu:has\-Upstream\-Node}{$n_p$}.
...
The neighbors of a node $n$ are represented in its \gls{kb} with the triples \triplet{$n$}{\concept{lmu}{hasUpperNode}}{$n_{parent}$} for the parent of $n$, and \triplet{$n$}{\concept{lmu}{hasLowerNode}}{$n_{child}$} for each child $n_{child}$ of $n$.
...
\end{itemize}
These mechanisms allow to add informations about a node by exchanging light messages containing partial representations, while enabling to remove outdated statements with the complete update. 
A particular node characteristic that is declared in the announce functionality is the type of data in which a node is interested, captured with the predicate \concept{edr}{isInterestedIn}, which is used in the data sending functionality.
...
When a node $n$ receives a new observation, either from its own sensors or lower nodes, $n$ executes the rules $r$ stored in its \gls{kb} if the description of $r$ contains \triplet{$r$}{edr:isRuleActive}{true}.

\paragraph{Deliver deduction:}
\label{par:deduction_delivery}
If the processing of an observation with rule $r$ by node $n$ leads to a deduction $\delta$, $\delta$ is sent to each node belonging to $\bigcup n_{consumer}$ where \triplet{$n_{consumer}$}{edr:consumesResult}{$r$} is in the \gls{kb} of $n$.
...
%In Chapter \textsection \ref{chap:experimentations}, the deduction delivery mechanism is used to manipulate the communication flow, which would not be possible if it was not a functionality independent from the interest announce. 

\paragraph{Send data:}
\label{par:data_transfer}
If the parent of node $n$ (denoted $n_{p}$) has declared its interest for the type of the new observation $\alpha_t$, the observation is forwarded toward $n_{p}$.
Observations are exchanged lazily: if a node $n$ receives an observation of type $\alpha_t$, and knows no other node interest in such type, the observation is not forwarded.
Such interest is represented in node $n$ \gls{kb} with the triplet \triplet{$n_{p}$}{\concept{edr}{is\-Interested\-In}}{$\alpha_t$}.
...
It the case where rule $r$ should be propagated towards node $n_{target}$ by $n$, the triplet \triplet{$r$}{edr:transferable\-To}{$n_{target}$} is present in $n$'s \gls{kb}.

\subsubsection{Controlling the propagation of nodes characteristics}
...
Let us consider a node $n$, with a characteristic represented by the property $hasCharacteristic$ and captured in its knowledge base such that \triplet{$n$}{$hasCharacteristic$}{$\nu$}, with $\nu$ either a literal or an individual denoting the value of the characteristic for $n$.
...
If the property $hasCharacteristic$ is such that \triplet{$hasCharacteristic$}{\concept{rdf}{type}}{\concept{edr}{Parent\-Announced\-Property}}, then the triple \triplet{$n$}{$hasCharacteristic$}{$\nu$} is part of the self description sent by the node $n$ to its parent because $hasCharacteristic$ is considered a relevant characteristic of $n$.

\paragraph{Announcing characteristic to a node's children:}
The announce mechanism from parent to children is quite similar to the one from children to parent, with the exception that children may be many, whereas by assumption, a node only has one parent.
...
\subsubsection{Propagating knowledge beyond direct neighbors}
...
If the characteristics of a the child $n_{child}$ of a node $n$ makes it adequate to apply a rule which is held by the parent of $n$ $n_{parent}$, but $n$ cannot apply the rule, $n_{parent}$ will not propagate the rule to $n$, preventing its eventual propagation to $n_{child}$.
...
Specifically, node $n$ proxying its parents capabilities towards its children means that $n$ announces these capabilities as its own to its children.
...
Let us assume that node $n$ has a child $n_{child}$, and that such child has a characteristic expressed by the triplet \triplet{$n_{child}$}{$has\-Cha\-racteri\-stic$}{$\nu$}, that should be proxied towards the parents of $n$.
Such information about the predicate $c$ is materialized by the triplet \triplet{$has\-Characteri\-stic$}{\concept{rdf}{type}}{\concept{edr}{Parent\-Proxied\-Property}}.
...
Therefore, the intelligence regarding rule deployment is located in the rules, and not in the nodes. 
The behavior of the algorithm at a global scale can thus be parametrized at a fine granularity, for each rule.
...
The envelope of rule $r$ is denoted $r^{envelope}$.
...
\Nicolas{L'impact de cette technique (présenté brièvement dans coopis) n'est pas mesuré dans les résultats, est-ce qu'on en garde la description ? Si non, mettre à jour l'algo}
...
Therefore, a simple query is sufficient to activate or deactivate rule modules related to deployment, and to do so for all the rules stored in a node's \gls{kb}.
...
That is why no activation is required when receiving a new rule, (1) on Fig. \ref{fig:edr_algo}. 
The rule deployment update, (3) on Fig. \ref{fig:edr_algo}, is performed by the reasoner.
...
That is why, for all the rules stored in the node's \gls{kb}, the deployment modules are activated upon the reception of a topology update, as seen in (2) on Fig. \ref{fig:edr_algo}.
...
If the placement rule needs to be updated due to the topology change, the new deployment is enforced by activating or propagating rules in compliance with the deductions and the \gls{edr} vocabulary, before deactivating the rules deployment modules (4) on Fig. \ref{fig:edr_algo}.
...
\Nicolas{Extension par rapport à coopis}
...
Considering this marking or not is up to the rule implementors: for instance, the strategy presented in Section \textsection \ref{sec:edrpt} takes it into account, so that each observation is at most processed once by each rule for performance issues.
...
The marking of observations with the \concept{edr}{used\-For\-Deduction\-By} property is shown on Fig. \ref{fig:edr_algo}, (5).
...
To tackle this issue, an object property is used: when a node transfers a rule $r$ to its child $n_{child}$, it adds the triple \triplet{$r$}{\concept{edr}{transferred\-To}}{$n_{child}$} to the rule description stored in its \gls{kb}.
...
\Nicolas{cf commentaire : garder le graphe de mots-clé ?}
...
%\begin{figure*}
%	\caption{Key terms overview}
%	\label{fig:edr_keywords}
%	\scalebox{0.8}{
%		\input{figures/edr_keywords.tex}
%	}
%\end{figure*}

\subsection{Implementing a deployment strategy based on property types with \edrt}
\label{subs:overview_edrt}
\Nicolas{Exprimer simplement l'objectif d'edrt en intro de la section, et joindre cette description plus détaillée avec son implémentation par les règles}
The purpose of \edrt is to bring rules as deep as possible in the topology, in order for them to be processed as soon as possible, while limiting unnecessary message exchanges.
...
We introduce $body_t(r_{x})=\{\gamma_{1},...,\gamma_{n'}\}$ and $head\_t(r_{x})=\{\delta_{1}, ..., \delta_{m'}\}$ where $\gamma_{i}$ designates the property type of $\Gamma_{i}$, and $\delta_{j}$ the property type of the deduction $\Delta_{j}$.
It should be noted that not all $\Gamma_{i}$ or $\Delta_{j}$ used in the rule are relevant to the \edrt approach.
\Nicolas{retrouver et remplacer toutes les références à rcomfort et aux noeuds de la topologie}
...
A translation of $R_{Visibility}$ in based on description logic is: $Location(?l) \land Presence(?l, ?o_1) \land ?o_1 = True \land Luminosity(?l, ?o_2) \land ?o_2 < 300L \land Machine(?m) $\\$ \land Activity(?m, ?o_3) \land ?o_3 = True \land locatedIn(?m, ?l) \rightarrow LowMachineVisibility(?m)$.
For this rule, the defined predicates behave as follows: $body_t(R_{Visibility})=\{Presence, Luminosity, Activity\}$, and $head\_t(R_{Visibility})= \{LowMachineVisibility\}$. 
$Location$ is a property type that is not considered by the deployment strategy implemented by \edrt.
For $R_{ColdChain}$, represented in description logic in Section \textsection \ref{subs:factory_use_case}, $body_t(R_{ColdChain})=\{Temperature, Activity\}$, and $head\_t(R_{conveyor})= \{ColdChainBroken\}$.
...
Observations and devices are described in each node's \gls{kb} using the IoT-O ontology for our experiments (\textit{c.f.} Section \textsection \ref{sec:experimentations}), but the proposed approach does not depend on the ontology used to describe data, as long as the same ontology is used to express the rules and their metadata. 
...
A node $n$ knows its parent in the network tree-like hierarchy, noted $Upper(n)$, and its children, referred to as $Lower(n)$. 
On Fig. \ref{fig:edrpt_deployment}, $Lower(G110)=\{R111, R112\}$, and $Upper(G110)=\{F100\}$.
...
For instance, on Fig. \ref{fig:edrpt_deployment}, $productions(G110) = \{Temperature, Luminosity, Thermostat\}$, with\\$own\_productions(G110) = \{Thermostat\}$.
...
In Fig. \ref{fig:edrpt_deployment}, R111 announced to G110 that it produced $temperature$, and G110 notified R111 of its interest for $temperature$ in order to receive observations.
...
\gls{iot} data is strongly bound to a spacio-temporal context \cite{Perera2014_context}, and the distribution of Fog nodes reflects the distribution of features observed by sensors.
From this hypothesis, it can be inferred that the context of a node is a subset of the context of its parent.
To illustrate this claim with $R_{Comfort}$ previously introduced, it means that if it is possible to apply $R_{Comfort}$ with luminosity and temperature observations collected by the same gateway, it is not necessary to compare the same luminosity observations with temperature observations collected elsewhere.
...
\gls{edr} is therefore suitable for rules exploiting this context by correlating data sharing an identical context, \textit{e.g.,} the correlation of temperature and luminosity in the context of a single room for $R_{Comfort}$.
\Nicolas{Update according to smart factory use case}
\begin{figure}
	\centering
...
The assumption we make entails that, since both R111 and R112 contexts contain enough information to process rule $R_{Comfort}$, the luminosity from R111 context and the temperature from R112 context will never be processed together by $R_{Comfort}$.

In the case of the G120 context, since neither R121 nor R122 produce the information necessary to process $R_{TemperatureGap}$, both nodes send their observations to G120.
The fact that G120 is the parent of both R121 and R122 is considered a hint that the context of R122 is closer to the context of R121 than, for instance, to the context of R112.
The proximity of context is associated to the distance of the closest common ancestor: R121 and R122 share a parent, while the closest common ancestor to R121 and R112 is F100, at a distance of 2 hops from both nodes.
Since R121 and R122 are closer to each other than R122 and R112, there is a higher chance for the thermostat observation from R122 to lead to a deduction based on $R_{TemperatureGap}$ when processed with temperature from R121 rather than R112.
...
On Fig. \ref{fig:contextuality}, the G110 context partially includes the R111 and R112 contexts, since only temperature values are propagated to G110, and not presence values.
Since G110 applies $R_{TemperatureGap}$, R111 and R112 provide it with temperature observations, which it processes with its own thermostat value observations.
...
For instance, let us consider two sensors producing respectively observations of types $\alpha_1$ and $\alpha_2$, connected to the same node $n$, and a rule $r$ consuming $\alpha_1$ and $\alpha_2$.
\edrt will eventually deploy $r$ on $n$, and none of the observations of type $\alpha_1$ and $\alpha_2$ produced by $n$ will be processed by $r$ outside of the context of $n$.
...
%Moreover, such aggregation rules might need to consider data in a global context, which is a purpose achieved by the deployment on Cloud nodes as discussed in Section \textsection \ref{sec:analysis}.
...
When a node applies rule $r$, it is considered as producer of the $head\_t(r)$, and this production information is used for the deployment of any rule $r'$ such as $body_t(r')\cap head\_t(r)\neq \emptyset$.
However, a non stratified rule set where rules $r$ and $r'$ coexist such that $body_t(r')\subseteq head\_t(r)$ and $body_t(r)\subseteq head\_t(r')$ cannot be processed successfully by \gls{edr}, and neither $r$ nor $r'$ will be propagated or applied.
...
%This condition is expressed in Lst. \ref{lst:transferShape}, an extract of the SHACL shape constituting $r_{comfort}^{transfer}$.
%
%\begin{lstlisting}[float, caption=$r_{comfort}^{transfer}$ shape, label=lst:transferShape]
%SELECT $this
%	WHERE {
%	FILTER NOT EXISTS {
%		$this a lmu:Node ;
...
\begin{itemize}
	\item $n$ can no longer \textbf{propagate} $r$, \textit{i.e.} $\forall n'\in Lower(n), body_t(r_{x})\not\subset productions(n')$,
	\item $n$ is able to \textbf{apply} $r$, \textit{i.e.} $body_t(r_{x})\subset productions(n)$.
\end{itemize} 
...
Such a node is represented on Fig. \ref{fig:edrpt_deployment} with gray dashes connected to $R_{Comfort}$. 
...
It is characterized as such: $\exists \mathcal{P}$, such as $\forall n_c\in \mathcal{P}$, \triplet{$n$}{\concept{lmu}{hasDownstreamNode}}{$n_c$} and $\exists \{\rho_t, \rho'_t\} \subseteq body(r)$, \triplet{$n_c$}{\concept{edr}{producesDataOn}}{$\rho_t$} and $\neg\exists$ \triplet{$n_c$}{\concept{edr}{producesDataOn}}{$\rho'_t$}, and $\forall \rho_t \in body(r), \exists n_c\in \mathcal{P},$\triplet{$n_c$}{edr:producesDataOn}{$\rho_t$}.
%Lst. \ref{lst:activationShape} gives a SPARQL implementation of these conditions applied to $r_{comfort}^{activation}$. 
%
%\begin{lstlisting}[float, caption=$r_{comfort}^{activation}$ shape, label=lst:activationShape]
%SELECT $this
%	WHERE {
%		FILTER NOT EXISTS {
%			$this a lmu:HostNode.
...
%						adr:Temperature, adr:Luminosity.
%				}
%			}
%		}
%	}
...
The interest of $n$ for the consumption of the nodes $n'\in \mathcal{P}$ is announced, as it is captured by the \triplet{?interest}{\concept{edr}{announce\-To}}{?partialDataProvider} triple\Nicolas{Attention, basé sur le listing qui a été retiré}.
...
The interest of the rule originator $o$ is also denoted with \triplet{$o$}{\concept{edr}{consumesResult}}{$r$}.
...
%The SPARQL CONSTRUCT embedded in the SHACL rule for the $r_{comfort}^{activation}$ module is provided in Lst. \ref{lst:activationRule}.
...
That is why some elements characterizing the child nodes of the current node need to be recaptured in the WHERE clause of the $r_{comfort}^{activation}$ rule, while the \texttt{\$this} is already bound to the current node.

%\begin{lstlisting}[float, caption=$r_{comfort}^{activation}$ rule, label=lst:activationRule]
%CONSTRUCT {
...
If there exists a triple \triplet{$n'$}{\concept{edr}{interestedIn}}{$\rho_t$}, with $n'$ a remote node and $\rho_t$ an element of the rule $r$'s head $head(r)$, then the result transfer module infers that \triplet{$n'$}{edr:consumesResult}{$r$}. 
...
The propagation of $r_{TemperatureGap}$ in the illustrative deployment provided in Fig. \ref{fig:edrpt_deployment} is represented as a sequence diagram on Fig. \ref{fig:seq_temperaturegap}.

\begin{figure*}
	\centering
	\caption{Propagation of $r_{TemperatureGap}$}
...
\begin{itemize}
	\item If the rule $r$ is applicable by the current node, the productions of $n$ are updated by $r_{comfort}^{activation}$.
...
%Two sets of experimentations are then introduced, assessing two different implementations of \edrt, respectively in Section \textsection \ref{sec:initial_impl} and Section \textsection \ref{sec:second_implementation}.
%These two groups of experimentations have been initially published in \cite{Seydoux2018_itl}, \cite{Seydoux2018_wi} and \cite{Seydoux2018_coopis}.
...
The considered topology comes from the smart building use case described in Section \textsection \ref{sec:distribution_use_case}, with a hierarchical deployment of nodes in a floor, galleries and rooms, respectively designated as FXXX, GXXX and RXXX on the figure.
...
\begin{itemize}
	...
	However, the feasibility of large scale experimentations on such decentralized environments is limited, since it requires multiple machines.
...
The topologies introduced in the use cases are simulated for the experimentations.
...
\Nicolas{Copié d'ailleurs. Refaire le lien avec le use case initialement introduit dans les motivations}
The rule's SHACL representation is available online\footnote{\url{https://w3id.org/laas-iot/edr/iiot/iiot.tar.gz}}
...
\Nicolas{Copied from initial implementation}
To measure how distribution impacts responsiveness, four topologies were distinguished, labeled d1 to d4 and further on simply denoted d*. Each of these topologies is constituted of 47 identical nodes, and processes data according to four rules, r1 to r4.
The difference between the four d* topologies is the location of sensors, as depicted in Fig. \ref{fig:dstar}. 
Sensors producing data of the type $\gamma_{1}$ are directly attached to the top node in d1, while they are attached to its children in d2. 
Since $body\_t(r1)=\{\gamma_{1}, \gamma_{4}\}$, $r1$ is applied at a maximum depth of 1 in d1, but is propagated to nodes of depth 2 in d2, hence a ``more decentralized'' execution is performed in d2 than in d1.
...
\begin{figure}
	...
	\Nicolas{Mettre à jour pour factory use case}
	\label{fig:dstar}
	\subcaption{d1}
	\scalebox{0.75}{
...
A factor to be considered is that this experiment is entierly run on the server which characteristics are given in Tab. \ref{tab:setup}: no constrained nodes are included in this evaluation.
It would be wrong to assume that the decentralization towards nodes of lesser computing power has no impact on the results, that is why this question is addressed in Section \textsection \ref{subs:factory_distribution}.
...
\begin{figure*}
	...
	\end{minipage}
%	\Centering
%	\caption{\Centering Distribution experiments, distributed reasoning}
%	\label{fig:factory_distribution_processed}
	\begin{minipage}{0.595\textwidth}
		\Centering
...
For the decentralized delivery mechanisms, where rules are propagated into the network according to the \edrt technique, the distribution has indeed an impact on deduction delivery delay, seen on Fig. \ref{fig:factory_distribution_processed}.
...
However, Fig. \ref{fig:factory_distribution_processed} shows a discrepancy between the simulation in a single-host and a multi-host-host environment, the latter actually including constrained nodes.
...
\Nicolas{Move the following explanation to a dedicated "Discussion" section}
An explanation for this phenomenon is the saturation of the Fog node passed a certain work load, the tipping point being crossed around d'3. 
...
Moreover, the nature of SHACL rules requires the node to evaluate every active rules when a new observation is received, event if the new information is not relevant for the new rule.
This prevents optimizations that were possible in the initial implementation, where rules were individually processed: the generality of the \gls{edr} approach is traded for the efficiency of the implementation. 

\Nicolas{Garder la suite ?}
...
\begin{itemize}
	...
	\item The SHACL engine used in our experimentations is described by its creators as "not really optimized for performance, just for correctness"\footnote{\url{https://github.com/TopQuadrant/shacl}}.
...
\begin{figure*}
	...
	\begin{minipage}{0.395\textwidth}
		\Centering
		\subcaption{Single-host execution}
...
%		\Centering
%		\subcaption{Decentralized execution (RPi 2)}
%		\label{fig:factory_scala_cip_rpi2}
%		\scalebox{1}{
%			\input{figures/results/factory/boxplot_scala_processed_rpi2.pgf}
%		}
...
%The average composition of delays is broken down in Fig. \ref{fig:breakout_delays_syn} and \ref{fig:breakout_delays_rpi}, after normalization: each bar represents the share of the delay type in the global delay.

\Nicolas{Voir graphe en commentaire, pas référencé dans l'analyse initiale}
%\begin{figure*}
%	\centering
%	\caption{Breakout of delays (normalized, single-host execution)}
%	\label{fig:breakout_delays_syn}
%	\scalebox{0.9}{
%		\input{figures/results/factory/breakout_delays_scala_syn.pgf}
%	}
...
It is notable that with a decentralized approach, such an increase of distribution has improved performances up to certain point, but when executed on constrained nodes, has eventually led to a \gls{qos} degradation.
This degradation is a drawback of \edrt that have been revealed by the conducted experimentations.
Not considering the capabilities of Fog nodes in the deployment process is a naive approach that reaches its limitations when overloading constrained nodes.
...
%\gls{edr} has been accepted for publication in \cite{Seydoux2018_coopis}.
...
%An early implementation of \edrt has been proposed in \cite{Seydoux2018_itl}, and a deeper description has been accepted for publication in \cite{Seydoux2018_wi}.

The genericity and the dynamicity of the \gls{edr} approach are achieved by design, while its scalability and the improvement brought by distribution for responsiveness have been measured through experimentations.
...
The genericity of the \edr approach enables such extensions to be developed without modfying the core algorithm.
