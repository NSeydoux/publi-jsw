...
For $R_{ColdChain}$, represented in description logic in Section \textsection \ref{subs:factory_use_case}, the predicates results are $body_t(R_{ColdChain})=\{Tem\-pe\-ra\-ture, Ac\-ti\-vi\-ty\}$, and $head_t(R_{conveyor})= \{Cold\-Chain\-Bro\-ken\}$.
...
The interest of $n$ for the consumption of the nodes $n'\in \mathcal{P}$ is announced, as it is captured by the \triplet{?interest}{\concept{edr}{announce\-To}}{?partialDataProvider} triple in the SHACL rule.
...
%\Nicolas{Attention r\_comfort}
That is why some elements characterizing the child nodes of the current node need to be recaptured in the WHERE clause of the $r_{Visibility}^{activation}$ rule, while the \texttt{\$this} is already bound to the current node.
...
\Nicolas{Garder le diagramme de séquence ? si oui, adapter au use case}
...
Since $body_t(r1)=\{\gamma_{1}, \gamma_{4}\}$, $r1$ is applied at a maximum depth of 1 in d1, but is propagated to nodes of depth 2 in d2, hence a ``more decentralized'' execution is performed in d2 than in d1.
