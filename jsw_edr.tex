...
\Nicolas{À ramener dans les hypothèses : A node has two types of neighbors: its parent, and its children, and since the parent is unique (according to our assumptions) while the children are potentially many, two approaches are devised.}
...
This raw data is enriched thanks to a SPARQL-Generate query, a sample of which is shown in Lst. \ref{lst:enrichment}\footnote{\added{The complete query is available in the source code of EDR: node/\-queries/\-data/\-enrich\_data.sparql}}. 
...
\Nicolas{Refaire la transition}, we proposed \edrt, an \gls{edr} refinement implementing a deployment strategy dedicated to \textbf{reducing delays} for transmitting deductions to applications.
